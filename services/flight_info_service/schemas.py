...
class LocationDetail(BaseModel):
    ...
    airport_code: str = Field(..., regex=r"^[A-Z]{3}$", description="3-letter IATA airport code")

class FlightBase(BaseModel):
    # DÜZELTİLDİ: (Eksik 8) AANNNN formatı için Regex doğrulaması
    flight_number: str = Field(..., regex=r"^[A-Z]{2}\d{4}$", description="Flight number in AANNNN format")

...
def assign_seats(passengers: List[Dict], seat_map: Dict) -> List[Dict]:
    ...
    # mark occupied based on existing assignments
    for p in passengers:
    ...
    # build seats by row preserving order
    seats_by_row = defaultdict(list)
    for s in seat_map['seats']:
        seats_by_row[s['row']].append(s['seat_id'])

    # helper: find contiguous block in a row with same seat type
    ...
    # first, try to seat affiliated groups together
    for p in passengers:
        if p['id'] in handled:
            continue
        group_ids = [p['id']]
        ...
        # determine seat_type preference: majority or p's seat_type
        ...
        for row in sorted(seats_by_row.keys()):
            ...
            if block:
                ...
                placed = True
                break
        # if not placed, leave for later (individual assignment)
    # assign remaining individually
    # build free pools per seat type in row-major order
...
def select_pilots(pool: List[Dict], vehicle: str, distance: int) -> List[Dict]:
    ...
    # fill up to 3 pilots total with more seniors/juniors then at most 2 trainees
